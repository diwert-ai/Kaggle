...
def create_model(model_dir, num_classes, in_channels=5):
    checkpoint = torch.load(f'{model_dir}/model.pth', map_location=device)
    conf = Config(checkpoint['conf'])
    conf.pretrained = False
    model = ModelWrapper(conf, num_classes,in_channels=in_channels)
...
def pad_mask(conf, mask):
    ...
    padded = np.zeros((conf.h_image_size, conf.w_image_size), dtype=mask.dtype)
    dh = conf.h_image_size - mask.shape[0]
    dw = conf.w_image_size - mask.shape[1]
    top = dh//2
    left = dw//2
