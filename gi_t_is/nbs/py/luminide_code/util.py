...
class LossHistory:
    def __init__(self, save_dir=None):
        ...
        self.hist_filename=f"history_{datetime.now().strftime('%b%d_%H-%M-%S')}.csv"
    ...
    def save(self):
        ...
        save_path = self.hist_filename if self.save_dir == None else self.save_dir+self.hist_filename
...
def process_files(input_dir, img_dir, meta_df, class_names):
    ...
    pbar = tqdm(enumerate(img_files), total=len(img_files), desc='Maskgen ')
    for step, img_file in pbar:
...
def make_test_augmenter(conf):
    h_crop_size = round(conf.h_image_size*conf.crop_size)
    w_crop_size = round(conf.w_image_size*conf.crop_size)
    
    return  A.Compose([
        A.CenterCrop(height=h_crop_size, width=w_crop_size),
