...
def make_fold(mode='train-1'):
    if 'train' in mode:
        ...
        df_image = pd.read_csv(data_dir+'/train_image_level.csv')
        ...
        df_image['image']=df_image.id.apply(lambda x: x[:x.find('_image')])
        df_image['none']= df_image.label.apply(lambda x: 1 if(x.find('none')!=-1) else 0)
        ...
        df = df.merge(df_image[['image','none']], on='image')
...
def null_collate(batch):
    ...
    none = np.ascontiguousarray(np.stack(collate['none'])).astype(np.float32)
    collate['none']= torch.from_numpy(none)
...
def run_check_dataset():
    ...
    for t,batch in enumerate(loader):
        ...
        print('\t', batch['none'])
        print('\t')
        print('\t',batch['none'].shape, batch['none'].is_contiguous())
