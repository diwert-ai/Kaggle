...
def get_real_annotations(table):
    ...
    labels = table['LabelName'].values.astype(np.int)
...
def mean_average_precision_for_boxes(valid, preds, iou_threshold=0.5, exclude_not_in_annotations=False, verbose=True):
    """
    ...
    # valid.loc[:,'LabelName']=valid['LabelName'].astype(np.int32)
    # preds.loc[:,'LabelName']=preds['LabelName'].astype(np.int32)
    #---------------------------------------------
    ...
    for zz, label in enumerate(sorted(unique_classes)):
        ...
        if verbose:
            s1 = "{:2d} | {:.6f} | {:7d}".format(label, average_precision, int(num_annotations))
            print(s1)
