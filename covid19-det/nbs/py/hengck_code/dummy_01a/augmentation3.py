...
# #--- flip ---
def do_random_hflip(image, mask, annotate):
    if np.random.rand()>0.5:
        image = cv2.flip(image,1)
        mask = cv2.flip(mask,1)
        annotate[:,1]=1-annotate[:,1]
    return image, mask, annotate


# #--- geometric ---
def do_rotate_transform_annotate(annotate, transform, width, height):
    def tx(x,y):
        xx = transform[0,0]*x + transform[0,1]*y + transform[0,2]
        yy = transform[1,0]*x + transform[1,1]*y + transform[1,2]
        return xx,yy
    ...
    if 1: #remove invalid
        keep = (x>0)&(x<1)&(y>0)&(y<1)
        annotate = annotate[keep]
    return annotate

def do_transform_annotate(annotate, transform, width, height):
    def tx(x,y):
        xx = transform[0,0]*x + transform[0,1]*y + transform[0,2]
        yy = transform[1,0]*x + transform[1,1]*y + transform[1,2]
        return xx,yy
    ...
    annotate[:, 1] = x
    annotate[:, 2] = y
    annotate[:, 3] *= transform[0,0]
    annotate[:, 4] *= transform[1,1]
...
def do_random_rotate(image, mask, annotate, mag=15 ):
    ...
    annotate = do_rotate_transform_annotate(annotate, transform, width, height)
    return image, mask, annotate


def do_random_scale( image, mask, annotate, mag=0.1 ):
    ...
    image = cv2.warpAffine( image, transform, (width, height), flags=cv2.INTER_LINEAR, borderMode=cv2.BORDER_CONSTANT, borderValue=0)
    mask = cv2.warpAffine( mask, transform, (width, height), flags=cv2.INTER_LINEAR, borderMode=cv2.BORDER_CONSTANT, borderValue=0)
    annotate = do_transform_annotate(annotate, transform, width, height)
    return image, mask, annotate


def do_random_stretch_y( image, mask, annotate, mag=0.25 ):
    s = 1 + np.random.uniform(-1, 1)*mag
    height, width = image.shape[:2]
    h = int(s*height)
    w = width
...
def do_random_stretch_x( image, mask, annotate, mag=0.25 ):
    s = 1 + np.random.uniform(-1, 1)*mag
    height, width = image.shape[:2]
    h = height
    w = int(s*width)
...
def do_random_shift( image, mask, annotate, mag=32 ):
    b = mag
    height, width = image.shape[:2]
    ...
    # ---
    if 1:
        x = annotate[:, 1]-(x-b)/width
        y = annotate[:, 2]-(y-b)/height
        annotate[:, 1] = x
        annotate[:, 2] = y
